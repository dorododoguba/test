...
class Paypal_For_Edd {
    ...
    public function __construct() {
        ...
        add_action('paypal_for_edd_api_ipn_handler', array($this, 'paypal_for_edd_api_ipn_handler'));
    ...
    public function paypal_for_edd_api_ipn_handler() {

        require_once plugin_dir_path(dirname(__FILE__)) . 'includes/class-pal-for-edd-paypal-listner.php';
        $PayPal_For_EDD_PayPal_listner = new PayPal_For_EDD_PayPal_listner();
        if ($PayPal_For_EDD_PayPal_listner->check_ipn_request()) {
            $PayPal_For_EDD_PayPal_listner->successful_request($IPN_status = true);
        } else {
            $PayPal_For_EDD_PayPal_listner->successful_request($IPN_status = false);
        }
    }
